...
function Cards({ videoName, isAnalised, videoUrl, videoId }) {
  const [thumbnailUrl, setThumbnailUrl] = useState(null);
  const truncatedName = videoName.length > 15 ? videoName.substring(0, 15) + '...' : videoName;
  ...
  return (
    ...
    <div className={styles.videoFlex}>
      ...
      <div>
        <h6 style={{cursor: 'pointer', maxWidth: '150px', fontStyle: 'italic'}}>{truncatedName}</h6>
        <p>{isAnalised}</p>
      </div>
    </div>
    </>
      ...
      <div className={styles.column}>
        {videos.map(video => (
          <Cards 
            ...
            isAnalised={video.analise ? 'Analizado' : 'Não analisado'} 
            videoUrl={video.url}
            videoId={video.id}
          />
        ))}
